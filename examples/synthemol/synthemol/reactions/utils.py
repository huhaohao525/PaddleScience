...
def set_all_building_blocks(reactions: tuple[Reaction], building_blocks:
    set[str]) ->None:
...
def load_and_set_allowed_reaction_building_blocks(reactions: tuple[Reaction
    ], reaction_to_reactant_to_building_blocks_path: Path) ->None:
    ...
    for reaction in reactions:
        for reactant_index, reactant in enumerate(reaction.reactants):
            reactant.allowed_building_blocks = (
                reaction_to_reactant_to_building_blocks[reaction.id][
                reactant_index])
