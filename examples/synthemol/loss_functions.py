...
def get_loss_func(args: TrainArgs) ->Callable:
    ...
    supported_loss_functions = {'regression': {'mse': paddle.nn.MSELoss(
        ...
        normal_mve, 'evidential': evidential_loss}, 'classification': {
        'binary_cross_entropy': paddle.nn.BCEWithLogitsLoss(reduction=
        'none'), 'mcc': mcc_class_loss, 'dirichlet': dirichlet_class_loss},
        'multiclass': {'cross_entropy': paddle.nn.CrossEntropyLoss(
        reduction='none'), 'mcc': mcc_multiclass_loss, 'dirichlet':
        dirichlet_multiclass_loss}, 'spectra': {'sid': sid_loss,
...
def bounded_mse_loss(predictions: paddle.to_tensor, targets: paddle.
    to_tensor, less_than_target: paddle.to_tensor, greater_than_target:
    ...
    """
    predictions = paddle.where(condition=paddle.logical_and(x=predictions <
        targets, y=less_than_target), x=targets, y=predictions)
    predictions = paddle.where(condition=paddle.logical_and(x=predictions >
        targets, y=greater_than_target), x=targets, y=predictions)
    return paddle.nn.functional.mse_loss(input=predictions, label=targets,
        reduction='none')


def mcc_class_loss(predictions: paddle.to_tensor, targets: paddle.to_tensor,
    data_weights: paddle.to_tensor, mask: paddle.to_tensor) ->paddle.to_tensor:
    """
    ...
    TN = paddle.sum((1 - targets) * (1 - predictions) * data_weights * mask,
        axis=0)
    loss = 1 - (TP * TN - FP * FN) / paddle.sqrt(x=(TP + FP) * (TP + FN) *
        (TN + FP) * (TN + FN))
    return loss


def mcc_multiclass_loss(predictions: paddle.to_tensor, targets: paddle.
    to_tensor, data_weights: paddle.to_tensor, mask: paddle.to_tensor
    ) ->paddle.to_tensor:
    """
    ...
    """
    torch_device = predictions.place
...
def sid_loss(model_spectra: paddle.to_tensor, target_spectra: paddle.
    to_tensor, mask: paddle.to_tensor, threshold: float=None
    ) ->paddle.to_tensor:
    """
    ...
    """
    torch_device = model_spectra.place
    zero_sub = paddle.zeros_like(x=model_spectra)
    one_sub = paddle.ones_like(x=model_spectra)
    if threshold is not None:
        threshold_sub = paddle.full(shape=tuple(model_spectra.shape),
            fill_value=threshold)
        model_spectra = paddle.where(condition=model_spectra < threshold, x
            =threshold_sub, y=model_spectra)
    ...
    model_spectra = paddle.divide(x=model_spectra, y=paddle.to_tensor(
        sum_model_spectra))
    ...
    loss = paddle.multiply(x=paddle.log(x=paddle.divide(x=model_spectra, y=
        paddle.to_tensor(target_spectra))), y=paddle.to_tensor(model_spectra)
        ) + paddle.multiply(x=paddle.log(x=paddle.divide(x=target_spectra,
        y=paddle.to_tensor(model_spectra))), y=paddle.to_tensor(target_spectra)
        )
    return loss


def wasserstein_loss(model_spectra: paddle.to_tensor, target_spectra:
    paddle.to_tensor, mask: paddle.to_tensor, threshold: float=None
    ) ->paddle.to_tensor:
    """
...
def normal_mve(pred_values, targets):
    ...
    """
    pred_means, pred_var = paddle_aux.split(x=pred_values, num_or_sections=
        tuple(pred_values.shape)[1] // 2, axis=1)
    return paddle.log(x=2 * np.pi * pred_var) / 2 + (pred_means - targets
        ) ** 2 / (2 * pred_var)
...
def dirichlet_class_loss(alphas, target_labels, lam=0):
    ...
    :return: Loss
    """
    torch_device = alphas.place
    num_tasks = tuple(target_labels.shape)[1]
    num_classes = 2
    alphas = paddle.reshape(x=alphas, shape=(tuple(alphas.shape)[0],
        num_tasks, num_classes))
    y_one_hot = paddle.eye(num_rows=num_classes)[target_labels.astype(dtype
        ='int64')]
...
def dirichlet_common_loss(alphas, y_one_hot, lam=0):
    ...
    ln_alpha = paddle.lgamma(x=S_alpha) - paddle.sum(x=paddle.lgamma(x=
        alpha_hat), axis=-1, keepdim=True)
    ln_beta = paddle.sum(x=paddle.lgamma(x=beta), axis=-1, keepdim=True
        ) - paddle.lgamma(x=S_beta)
    dg_alpha = paddle.digamma(x=alpha_hat)
    dg_S_alpha = paddle.digamma(x=S_alpha)
    KL = ln_alpha + ln_beta + paddle.sum(x=(alpha_hat - beta) * (dg_alpha -
        dg_S_alpha), axis=-1, keepdim=True)
    KL = lam * KL
    loss = SOS + KL
...
def evidential_loss(pred_values, targets, lam=0, epsilon=1e-08):
    ...
    :return: Loss
    """
    mu, v, alpha, beta = paddle_aux.split(x=pred_values, num_or_sections=
        tuple(pred_values.shape)[1] // 4, axis=1)
    twoBlambda = 2 * beta * (1 + v)
    nll = 0.5 * paddle.log(x=np.pi / v) - alpha * paddle.log(x=twoBlambda) + (
        alpha + 0.5) * paddle.log(x=v * (targets - mu) ** 2 + twoBlambda
        ) + paddle.lgamma(x=alpha) - paddle.lgamma(x=alpha + 0.5)
    L_NLL = nll
