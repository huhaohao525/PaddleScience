...
def preprocess_smiles_columns(path: str, smiles_columns: Union[str, List[
    str]]=None, number_of_molecules: int=1) ->List[str]:
    """
...
def get_checkpoint_paths(checkpoint_path: Optional[str]=None,
    checkpoint_paths: Optional[List[str]]=None, checkpoint_dir: Optional[
    str]=None, ext: str='.pt') ->Optional[List[str]]:
    """
    ...
    """
    if sum(var is not None for var in [checkpoint_dir, checkpoint_path,
        checkpoint_paths]) > 1:
        raise ValueError(
...
class CommonArgs(Tap):
    ...
    def configure(self) ->None:
        self.add_argument('--gpu', choices=list(range(paddle.device.cuda.
            device_count())))
        self.add_argument('--features_generator', choices=
            get_available_features_generators())

    def process_args(self) ->None:
        self.checkpoint_paths = get_checkpoint_paths(checkpoint_path=self.
            checkpoint_path, checkpoint_paths=self.checkpoint_paths,
            checkpoint_dir=self.checkpoint_dir)
        if (self.features_generator is not None and 'rdkit_2d_normalized' in
            self.features_generator and self.features_scaling):
            raise ValueError(
        ...
        if (self.bond_features_path is not None and self.
...
class TrainArgs(CommonArgs):
    ...
    loss_function: Literal['mse', 'bounded_mse', 'binary_cross_entropy',
        'cross_entropy', 'mcc', 'sid', 'wasserstein', 'mve', 'evidential',
        'dirichlet'] = None
    ...
    split_type: Literal['random', 'scaffold_balanced', 'predetermined',
        'crossval', 'cv', 'cv-no-test', 'index_predetermined',
        'random_with_repeated_smiles'] = 'random'
    ...
    dropout: float = 0.0
    """Dropout probability."""
    activation: Literal['ReLU', 'LeakyReLU', 'PReLU', 'tanh', 'SELU', 'ELU'
        ] = 'ReLU'
    """Activation function."""
    ...
    def minimize_score(self) ->bool:
        ...
        return self.metric in {'rmse', 'mae', 'mse', 'cross_entropy',
            'binary_cross_entropy', 'sid', 'wasserstein', 'bounded_mse',
            'bounded_mae', 'bounded_rmse'}

    @property
    def use_input_features(self) ->bool:
        """Whether the model is using additional molecule-level features."""
        return (self.features_generator is not None or self.features_path
             is not None or self.phase_features_path is not None)
    ...
    def process_args(self) ->None:
        ...
        self.smiles_columns = preprocess_smiles_columns(
            path=self.data_path, smiles_columns=self.smiles_columns,
        ...
        if self.checkpoint_paths is not None and len(self.checkpoint_paths
            ) > 0:
        ...
        for metric in self.metrics:
            if not any([self.dataset_type == 'classification' and metric in
                ['auc', 'prc-auc', 'accuracy', 'binary_cross_entropy', 'f1',
                'mcc'], self.dataset_type == 'regression' and metric in [
                'rmse', 'mae', 'mse', 'r2', 'bounded_rmse', 'bounded_mae',
                'bounded_mse'], self.dataset_type == 'multiclass' and 
                metric in ['cross_entropy', 'accuracy', 'f1', 'mcc'], self.
                dataset_type == 'spectra' and metric in ['sid', 'wasserstein']]
                ):
                raise ValueError(
        ...
        if self.loss_function != 'bounded_mse' and any(metric in [
            'bounded_mse', 'bounded_rmse', 'bounded_mae'] for metric in
            self.metrics):
            raise ValueError(
        ...
        if self.features_only and not (self.features_generator or self.
            features_path):
            raise ValueError(
        ...
        if not (self.split_type == 'predetermined') == (self.folds_file is not
            None) == (self.test_fold_index is not None):
            raise ValueError(
                'When using predetermined split type, must provide folds_file and test_fold_index.'
                )
        if not (self.split_type == 'crossval') == (self.crossval_index_dir
             is not None):
            raise ValueError(
        ...
        if self.split_sizes is None:
            if (self.separate_val_path is None and self.separate_test_path is
                None):
            ...
            if (self.separate_val_path is None and self.separate_test_path is
                ...
                if len(self.split_sizes) == 2:
                    self.split_sizes = [self.split_sizes[0], 0.0, self.
                        split_sizes[1]]
                if self.split_sizes[0] == 0.0:
                    raise ValueError(
                ...
                if len(self.split_sizes) == 2:
                    self.split_sizes = [self.split_sizes[0], self.
                        split_sizes[1], 0.0]
                if self.split_sizes[0] == 0.0:
                    raise ValueError(
                        'Provided split size for train split must be nonzero.')
        ...
        for features_argument, base_features_path, val_features_path, test_features_path in [
            ('`--features_path`', self.features_path, self.
            separate_val_features_path, self.separate_test_features_path),
            ('`--phase_features_path`', self.phase_features_path, self.
            separate_val_phase_features_path, self.
            separate_test_phase_features_path), (
            '`--atom_descriptors_path`', self.atom_descriptors_path, self.
            separate_val_atom_descriptors_path, self.
            separate_test_atom_descriptors_path), ('`--bond_features_path`',
            self.bond_features_path, self.separate_val_bond_features_path,
            self.separate_test_bond_features_path)]:
            if base_features_path is not None:
                if (self.separate_val_path is not None and 
                    val_features_path is None):
                    raise ValueError(
                ...
                if (self.separate_test_path is not None and 
                    test_features_path is None):
                    raise ValueError(
        ...
        if (self.overwrite_default_atom_features and self.atom_descriptors !=
            'feature'):
            raise NotImplementedError(
        ...
        if (self.overwrite_default_bond_features and self.
            bond_features_path is None):
            raise ValueError(
        ...
        if self.target_weights is not None:
            ...
            self.target_weights = [(w / avg_weight) for w in self.
                target_weights]
...
class PredictArgs(CommonArgs):
    ...
    uncertainty_method: Literal['mve', 'ensemble', 'evidential_epistemic',
        'evidential_aleatoric', 'evidential_total', 'classification',
        'dropout', 'spectra_roundrobin'] = None
    """The method of calculating uncertainty."""
    calibration_method: Literal['zscaling', 'tscaling', 'zelikman_interval',
        'mve_weighting', 'platt', 'isotonic'] = None
    ...
    def process_args(self) ->None:
        ...
        if self.uncertainty_method == 'dropout' and version.parse(paddle.
            __version__) < version.parse('1.9.0'):
            raise ValueError(
                'Dropout uncertainty is only supported for pytorch versions >= 1.9.0'
                )
        self.smiles_columns = preprocess_smiles_columns(
            path=self.test_path, smiles_columns=self.smiles_columns,
            number_of_molecules=self.number_of_molecules)
        ...
        if (self.calibration_interval_percentile <= 1 or self.
            calibration_interval_percentile >= 100):
            raise ValueError(
        ...
        for features_argument, base_features_path, cal_features_path in [(
            '`--features_path`', self.features_path, self.
            calibration_features_path), ('`--phase_features_path`', self.
            phase_features_path, self.calibration_phase_features_path), (
            '`--atom_descriptors_path`', self.atom_descriptors_path, self.
            calibration_atom_descriptors_path), ('`--bond_features_path`',
            self.bond_features_path, self.calibration_bond_features_path)]:
            if (base_features_path is not None and self.calibration_path is not
                None and cal_features_path is None):
                raise ValueError(
...
class HyperoptArgs(TrainArgs):
    ...
    def process_args(self) ->None:
        ...
        supported_keywords = ['basic', 'learning_rate',
            'linked_hidden_size', 'all', 'activation', 'aggregation',
            'aggregation_norm', 'batch_size', 'depth', 'dropout',
            'ffn_hidden_size', 'ffn_num_layers', 'final_lr', 'hidden_size',
            'init_lr', 'max_lr', 'warmup_epochs']
        supported_parameters = ['activation', 'aggregation',
            'aggregation_norm', 'batch_size', 'depth', 'dropout',
            'ffn_hidden_size', 'ffn_num_layers', 'final_lr_ratio',
            'hidden_size', 'init_lr_ratio', 'linked_hidden_size', 'max_lr',
            'warmup_epochs']
        ...
        if 'basic' in self.search_parameter_keywords:
            search_parameters.update(['depth', 'ffn_num_layers', 'dropout',
                'linked_hidden_size'])
        if 'learning_rate' in self.search_parameter_keywords:
            search_parameters.update(['max_lr', 'init_lr_ratio',
        ...
        if 'linked_hidden_size' in search_parameters and ('hidden_size' in
            search_parameters or 'ffn_hidden_size' in search_parameters):
...
class SklearnTrainArgs(TrainArgs):
    ...
    impute_mode: Literal['single_task', 'median', 'mean', 'linear', 'frequent'
        ] = None
...
class SklearnPredictArgs(Tap):
    ...
    def process_args(self) ->None:
        ...
        self.checkpoint_paths = get_checkpoint_paths(checkpoint_path=self.
            checkpoint_path, checkpoint_paths=self.checkpoint_paths,
            checkpoint_dir=self.checkpoint_dir, ext='.pkl')
