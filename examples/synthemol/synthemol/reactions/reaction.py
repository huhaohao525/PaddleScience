...
class Reaction:
    ...
    def __init__(self, reactants: list[QueryMol], product: QueryMol,
        ...
        self.reaction_smarts = (
            f"{'.'.join(f'({reactant.smarts_with_atom_mapping})' for reactant in self.reactants)}>>({self.product.smarts_with_atom_mapping})"
            )
    ...
    def get_reactant_matches(self, smiles: str) ->list[int]:
        ...
        """
        return [reactant_index for reactant_index, reactant in enumerate(
            self.reactants) if reactant.has_match(smiles)]
    ...
    def run_reactants(self, reactants: list[MOLECULE_TYPE]) ->tuple[tuple[
        Chem.Mol, ...], ...]:
        ...
        """
        return self.reaction.RunReactants([convert_to_mol(reactant, add_hs=
            True) for reactant in reactants])
