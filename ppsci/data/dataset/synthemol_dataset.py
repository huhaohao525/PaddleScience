...
class Featurization:
    ...
    def __init__(self):
        ...
        self.ATOM_FEATURES = {'atomic_num': list(range(self.MAX_ATOMIC_NUM)
            ), 'degree': [0, 1, 2, 3, 4, 5], 'formal_charge': [-1, -2, 1, 2,
            0], 'chiral_tag': [0, 1, 2, 3], 'num_Hs': [0, 1, 2, 3, 4],
            'hybridization': [Chem.rdchem.HybridizationType.SP, Chem.rdchem
            .HybridizationType.SP2, Chem.rdchem.HybridizationType.SP3, Chem
            .rdchem.HybridizationType.SP3D, Chem.rdchem.HybridizationType.
            SP3D2]}
        self.PATH_DISTANCE_BINS = list(range(10))
        self.THREE_D_DISTANCE_MAX = 20
        self.THREE_D_DISTANCE_STEP = 1
        self.THREE_D_DISTANCE_BINS = list(range(0, self.
            THREE_D_DISTANCE_MAX + 1, self.THREE_D_DISTANCE_STEP))
        self.ATOM_FDIM = sum(len(choices) + 1 for choices in self.
            ATOM_FEATURES.values()) + 2
    ...
    def atom_features(self, atom: Chem.rdchem.Atom, functional_groups: List[int]=None
        ...
        if atom is None:
            ...
            features = self.onek_encoding_unk(atom.GetAtomicNum() - 1, self.
                ATOM_FEATURES['atomic_num']) + self.onek_encoding_unk(atom.
                GetTotalDegree(), self.ATOM_FEATURES['degree']
                ) + self.onek_encoding_unk(atom.GetFormalCharge(), self.
                ATOM_FEATURES['formal_charge']) + self.onek_encoding_unk(int(atom.
                GetChiralTag()), self.ATOM_FEATURES['chiral_tag']
                ) + self.onek_encoding_unk(int(atom.GetTotalNumHs()), self.
                ATOM_FEATURES['num_Hs']) + self.onek_encoding_unk(int(atom.
                GetHybridization()), self.ATOM_FEATURES['hybridization']) + [
    ...
    def bond_features(self, bond: Chem.rdchem.Bond) ->List[Union[bool, int, float]]:
        ...
        if bond is None:
            ...
            bt = bond.GetBondType()
            fbond = [0, bt == Chem.rdchem.BondType.SINGLE, bt == Chem.rdchem.
                BondType.DOUBLE, bt == Chem.rdchem.BondType.TRIPLE, bt == Chem.
                rdchem.BondType.AROMATIC, bond.GetIsConjugated() if bt is not
                None else 0, bond.IsInRing() if bt is not None else 0]
            fbond += self.onek_encoding_unk(int(bond.GetStereo()), list(range(6)))
        return fbond

    def get_atom_fdim(self, overwrite_default_atom: bool=False, is_reaction: bool=False) ->int:
        ...
        if self.REACTION_MODE:
            return (not overwrite_default_atom
                ) * self.ATOM_FDIM + is_reaction * self.EXTRA_ATOM_FDIM
        else:
            return (not overwrite_default_atom
                ) * self.ATOM_FDIM + self.EXTRA_ATOM_FDIM

    def get_bond_fdim(self, atom_messages: bool=False, overwrite_default_bond: bool=
        False, overwrite_default_atom: bool=False, is_reaction: bool=False) ->int:
        ...
        """
        if self.REACTION_MODE:
            return (not overwrite_default_bond
                ) * self.BOND_FDIM + is_reaction * self.EXTRA_BOND_FDIM + (not
                atom_messages) * self.get_atom_fdim(overwrite_default_atom=
                overwrite_default_atom, is_reaction=is_reaction)
        else:
            return (not overwrite_default_bond
                ) * self.BOND_FDIM + self.EXTRA_BOND_FDIM + (not atom_messages
                ) * self.get_atom_fdim(overwrite_default_atom=overwrite_default_atom,
...
class MolGraph:
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        atom_features_extra=None, bond_features_extra=None, 
        ...
        if type(mol) == str:
            if self.is_reaction:
                mol = self.Featuri.make_mol(mol.split('>')[0], self.is_explicit_h, self.
                    is_adding_hs), make_mol(mol.split('>')[-1], self.
                    is_explicit_h, self.is_adding_hs)
            else:
        ...
        if not self.is_reaction:
            ...
            if atom_features_extra is not None:
                if overwrite_default_atom_features:
                    self.f_atoms = [descs.tolist() for descs in
                        atom_features_extra]
                else:
                    self.f_atoms = [(f_atoms + descs.tolist()) for f_atoms,
                        descs in zip(self.f_atoms, atom_features_extra)]
            self.n_atoms = len(self.f_atoms)
            if atom_features_extra is not None and len(atom_features_extra
                ) != self.n_atoms:
                raise ValueError(
            ...
            for a1 in range(self.n_atoms):
                for a2 in range(a1 + 1, self.n_atoms):
                    ...
                    self.n_bonds += 2
            if bond_features_extra is not None and len(bond_features_extra
                ) != self.n_bonds / 2:
                raise ValueError(
            ...
            if self.reaction_mode in ['reac_diff', 'prod_diff', 'reac_prod']:
                f_atoms_reac = [self.Featuri.atom_features(atom) for atom in mol_reac.
                    GetAtoms()] + [atom_features_zeros(mol_prod.
                    GetAtomWithIdx(index)) for index in pio]
                f_atoms_prod = [(self.Featuri.atom_features(mol_prod.GetAtomWithIdx(
                    ri2pi[atom.GetIdx()])) if atom.GetIdx() not in rio else
                    atom_features_zeros(atom)) for atom in mol_reac.GetAtoms()
                    ] + [self.Featuri.atom_features(mol_prod.GetAtomWithIdx(index)) for
                    index in pio]
            else:
                f_atoms_reac = [self.Featuri.atom_features(atom) for atom in mol_reac.
                    GetAtoms()] + [self.Featuri.atom_features(mol_prod.GetAtomWithIdx(
                    index)) for index in pio]
                f_atoms_prod = [(self.Featuri.atom_features(mol_prod.GetAtomWithIdx(
                    ri2pi[atom.GetIdx()])) if atom.GetIdx() not in rio else
                    self.Featuri.atom_features(atom)) for atom in mol_reac.GetAtoms()] + [
                    self.Featuri.atom_features(mol_prod.GetAtomWithIdx(index)) for index in
                    pio]
            if self.reaction_mode in ['reac_diff', 'prod_diff',
                'reac_diff_balance', 'prod_diff_balance']:
                f_atoms_diff = [list(map(lambda x, y: x - y, ii, jj)) for 
                    ii, jj in zip(f_atoms_prod, f_atoms_reac)]
            if self.reaction_mode in ['reac_prod', 'reac_prod_balance']:
                self.f_atoms = [(x + y[self.Featuri.MAX_ATOMIC_NUM + 1:]) for x,
                    y in zip(f_atoms_reac, f_atoms_prod)]
            elif self.reaction_mode in ['reac_diff', 'reac_diff_balance']:
                self.f_atoms = [(x + y[self.Featuri.MAX_ATOMIC_NUM + 1:]) for x,
                    y in zip(f_atoms_reac, f_atoms_diff)]
            elif self.reaction_mode in ['prod_diff', 'prod_diff_balance']:
                self.f_atoms = [(x + y[self.Featuri.MAX_ATOMIC_NUM + 1:]) for x,
                    y in zip(f_atoms_prod, f_atoms_diff)]
            self.n_atoms = len(self.f_atoms)
            ...
            for a1 in range(self.n_atoms):
                for a2 in range(a1 + 1, self.n_atoms):
                    if a1 >= n_atoms_reac and a2 >= n_atoms_reac:
                        bond_prod = mol_prod.GetBondBetweenAtoms(pio[a1 -
                            n_atoms_reac], pio[a2 - n_atoms_reac])
                        if self.reaction_mode in ['reac_prod_balance',
                            'reac_diff_balance', 'prod_diff_balance']:
                            bond_reac = bond_prod
                        else:
                            bond_reac = None
                        ...
                        bond_reac = None
                        if a1 in ri2pi.keys():
                            bond_prod = mol_prod.GetBondBetweenAtoms(ri2pi[
                                a1], pio[a2 - n_atoms_reac])
                        else:
                            bond_prod = None
                    else:
                        ...
                        if a1 in ri2pi.keys() and a2 in ri2pi.keys():
                            bond_prod = mol_prod.GetBondBetweenAtoms(ri2pi[
                                a1], ri2pi[a2])
                        elif self.reaction_mode in ['reac_prod_balance',
                            'reac_diff_balance', 'prod_diff_balance']:
                    ...
                    if self.reaction_mode in ['reac_diff', 'prod_diff',
                        'reac_diff_balance', 'prod_diff_balance']:
                        f_bond_diff = [(y - x) for x, y in zip(f_bond_reac,
                            f_bond_prod)]
                    if self.reaction_mode in ['reac_prod', 'reac_prod_balance'
                        ]:
                        f_bond = f_bond_reac + f_bond_prod
                    elif self.reaction_mode in ['reac_diff',
                        'reac_diff_balance']:
                        f_bond = f_bond_reac + f_bond_diff
                    elif self.reaction_mode in ['prod_diff',
                        'prod_diff_balance']:
...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        self.Featuri = Featurization()
        self.overwrite_default_atom_features = mol_graphs[0
            ].overwrite_default_atom_features
        self.overwrite_default_bond_features = mol_graphs[0
            ].overwrite_default_bond_features
        self.is_reaction = mol_graphs[0].is_reaction
        self.atom_fdim = self.Featuri.get_atom_fdim(overwrite_default_atom=self.
            overwrite_default_atom_features, is_reaction=self.is_reaction)
        self.bond_fdim = self.Featuri.get_bond_fdim(overwrite_default_bond=self.
            overwrite_default_bond_features, overwrite_default_atom=self.
            overwrite_default_atom_features, is_reaction=self.is_reaction)
        self.n_atoms = 1
        self.n_bonds = 1
        self.a_scope = []
        ...
        self.a2b = paddle.to_tensor(data=[(a2b[a] + [0] * (self.
            max_num_bonds - len(a2b[a]))) for a in range(self.n_atoms)],
            dtype='int64')
    ...
    def get_components(self, atom_messages: bool=False):
        ...
        if atom_messages:
            f_bonds = self.f_bonds[:, -get_bond_fdim(atom_messages=
                atom_messages, overwrite_default_atom=self.
                overwrite_default_atom_features, overwrite_default_bond=
    ...
    def get_b2b(self):
        ...
        if self.b2b is None:
            ...
            revmask = (b2b != self.b2revb.unsqueeze(axis=1).tile(
                repeat_times=[1, b2b.shape[1]])).astype(dtype='int64')
...
class StandardScaler:
    ...
    def fit(self, X: List[List[Optional[float]]]):
        ...
        self.means = np.where(np.isnan(self.means), np.zeros(tuple(self.
            means.shape)), self.means)
        self.stds = np.where(np.isnan(self.stds), np.ones(tuple(self.stds.
            shape)), self.stds)
        self.stds = np.where(self.stds == 0, np.ones(tuple(self.stds.shape)
            ), self.stds)
        return self
...
class MoleculeDatapoint:
    ...
    def __init__(self, smiles: List[str], targets: List[Optional[float]]=
        None, row: OrderedDict=None, data_weight: float=None, gt_targets:
        List[bool]=None, lt_targets: List[bool]=None, features: np.ndarray=
        None, features_generator: List[str]=None, phase_features: List[
        float]=None, atom_features: np.ndarray=None, atom_descriptors: np.
        ndarray=None, bond_features: np.ndarray=None,
        ...
        self.is_reaction_list = [self.Featuri.is_reaction(x) for x in self.is_mol_list]
        self.is_explicit_h_list = [self.Featuri.is_explicit_h(x) for x in self.is_mol_list]
        self.is_adding_hs_list = [self.Featuri.is_adding_hs(x) for x in self.is_mol_list]
                ...
                for m, reaction in zip(self.mol, self.is_reaction_list):
                    if not reaction:
                        if m is not None and m.GetNumHeavyAtoms() > 0:
                            ...
                            self.features.extend(np.zeros(len(
                                features_generator(Chem.MolFromSmiles('C')))))
                    elif m[0] is not None and m[1] is not None and m[0
                        ].GetNumHeavyAtoms() > 0:
        ...
        if self.atom_descriptors is not None:
            self.atom_descriptors = np.where(np.isnan(self.atom_descriptors
                ), replace_token, self.atom_descriptors)
        ...
        (self.raw_atom_descriptors, self.raw_atom_features, self.
            raw_bond_features
    ...
    def mol(self):
        ...
        """
        mol = make_mols(self.smiles, self.is_reaction_list, self.
            is_explicit_h_list, self.is_adding_hs_list)
        if cache_mol():
    ...
    def extend_features(self, features):
        ...
        self.features = np.append(self.features, features
            ) if self.features is not None else features
...
class MoleculeDataset(io.Dataset):
    ...
    def number_of_molecules(self) ->int:
        ...
        return: 
            The number of molecules.
        """
        return self._data[0].number_of_molecules if len(self._data
            ) > 0 else None
    ...
    def batch_graph(self):
        ...
        if self._batch_graph is None:
            ...
            for d in self._data:
                ...
                for s, m in zip(d.smiles, d.mol):
                    if s in SMILES_TO_GRAPH:
                        ...
                        if len(d.smiles) > 1 and (d.atom_features is not
                            None or d.bond_features is not None):
                            raise NotImplementedError(
                                'Atom descriptors are currently only supported with one molecule per input (i.e., number_of_molecules = 1).'
                                )
                        mol_graph = MolGraph(m, d.atom_features, d.
                            bond_features, overwrite_default_atom_features=
            ...
            self._batch_graph = [BatchMolGraph([g[i] for g in mol_graphs]) for
                i in range(len(mol_graphs[0]))]
    ...
    def features_size(self):
        ...
        """
        return len(self._data[0].features) if len(self._data
            ) > 0 and self._data[0].features is not None else None
    ...
    def atom_descriptors_size(self):
        ...
        """
        return len(self._data[0].atom_descriptors[0]) if len(self._data
            ) > 0 and self._data[0].atom_descriptors is not None else None
    ...
    def atom_features_size(self):
        ...
        """
        return len(self._data[0].atom_features[0]) if len(self._data
            ) > 0 and self._data[0].atom_features is not None else None
    ...
    def bond_features_size(self):
        ...
        """
        return len(self._data[0].bond_features[0]) if len(self._data
            ) > 0 and self._data[0].bond_features is not None else None

    def normalize_features(self, scaler: StandardScaler=None,
        replace_nan_token: int=0, scale_atom_descriptors: bool=False,
        ...
        """
        if (len(self._data) == 0 or self._data[0].features is None and not
            scale_bond_features and not scale_atom_descriptors):
            return None
        if scaler is None:
            if scale_atom_descriptors and not self._data[0
                ].atom_descriptors is None:
                features = np.vstack([d.raw_atom_descriptors for d in self.
                    _data])
            elif scale_atom_descriptors and not self._data[0
                ].atom_features is None:
        ...
        if scale_atom_descriptors and not self._data[0
            ...
            for d in self._data:
                d.set_features(scaler.transform(d.raw_features.reshape(1, -
                    1))[0])
        return scaler
...
class MoleculeSampler(io.Sampler):
    ...
    def __init__(self, dataset: MoleculeDataset, class_balance: bool=False,
...
def construct_molecule_batch(data: List[MoleculeDatapoint]):
    ...
    return (mol_graphs, features, targets, masks, atom_descriptors, 
        atom_features, bond_features, data_weights, lt_targets, gt_targets)
...
class MoleculeDataLoader(DataLoader):
    ...
    def __init__(self, dataset: MoleculeDataset, batch_size: int=50,
        ...
        super(MoleculeDataLoader, self).__init__(dataset=self._dataset, 
...
def make_mols(smiles: List[str], reaction_list: List[bool], keep_h_list:
    List[bool], add_h_list: List[bool]):
    """
    ...
    for s, reaction, keep_h, add_h in zip(smiles, reaction_list,
        keep_h_list, add_h_list):
        if reaction:
            mol.append(SMILES_TO_MOL[s] if s in SMILES_TO_MOL else (
                Featuri.make_mol(s.split('>')[0], keep_h, add_h), Featuri.make_mol(s.split(
                '>')[-1], keep_h, add_h)))
        else:
            mol.append(SMILES_TO_MOL[s] if s in SMILES_TO_MOL else Featuri.make_mol
                (s, keep_h, add_h))
    return mol
...
class MoleculeDatasetIter(io.IterableDataset):
    def __init__(
        ...
        self.data_loader = chemprop_build_data_loader(smiles, fingerprints, properties, shuffle, num_workers)
    ...
    def __iter__(self):
        for batch in self.data_loader:
            ...
            (mol_batch, features_batch, target_batch, mask_batch,
            atom_descriptors_batch, atom_features_batch,
            bond_features_batch, data_weights_batch, 
            lt_target_batch, gt_target_batch) = batch

            mask = paddle.to_tensor(data=mask_batch, dtype='bool')
            targets = paddle.to_tensor(data=[[(0 if x is None else x) for x in
                tb] for tb in target_batch])
            if self.args.target_weights is not None:
                target_weights = paddle.to_tensor(data=self.args.target_weights
                    ).unsqueeze(axis=0)
            else:
                target_weights = paddle.ones(shape=tuple(targets.shape)[1]
                    ).unsqueeze(axis=0)
            data_weights = paddle.to_tensor(data=data_weights_batch).unsqueeze(axis
                =1)
            ...
            yield (
                {
                    self.input_keys[0]: mol_batch, #paddle.to_tensor(mol_batch, dtype="float32"),
                    self.input_keys[1]: features_batch, #paddle.to_tensor(features_batch, dtype="float32"),
                    self.input_keys[2]: atom_descriptors_batch, #paddle.to_tensor(atom_descriptors_batch, dtype="float32"),
                    self.input_keys[3]: atom_features_batch, #paddle.to_tensor(atom_features_batch, dtype="float32"),
                    self.input_keys[4]: bond_features_batch, #paddle.to_tensor(bond_features_batch, dtype="float32"),
                },
                {
                    self.label_keys[0]: targets, #paddle.to_tensor(targets, dtype="float32"),
                    self.label_keys[1]: data_weights, #paddle.to_tensor(data_weights, dtype="float32"),
                    self.label_keys[2]: mask, #paddle.to_tensor(mask, dtype="float32"),
                    self.label_keys[3]: target_weights, #paddle.to_tensor(target_weights, dtype="float32"),
                },
                {},
            )
