...
def create_model_scoring_fn(model_path: Path, model_type: MODEL_TYPES,
    fingerprint_type: (FINGERPRINT_TYPES)=None, smiles_to_score: (
    dict[str, float])=None) ->Callable[[str], float]:
    ...
    if model_type != 'chemprop' and fingerprint_type is None:
        raise ValueError(
            'Must define fingerprint_type if using a scikit-learn model.')
    '''
    ...
    if model_type == 'chemprop':
        ...
        def model_scorer(smiles: str, fingerprint: (np.ndarray)=None
            ) ->float:
            return chemprop_predict_on_molecule_ensemble(models=models,
                smiles=smiles, fingerprint=fingerprint, scalers=scalers)
    else:
        '''
        ...
        def model_scorer(smiles: str, fingerprint: np.ndarray) ->float:
            return sklearn_predict_on_molecule_ensemble(models=models,
...
def save_generated_molecules(nodes: list[Node], building_block_id_to_smiles:
    dict[int, str], save_path: Path) ->None:
    ...
    for node in nodes:
        ...
        for reaction_index, reaction_log in enumerate(node.construction_log):
            ...
            reaction_num_to_max_reactant_num[reaction_num] = max(
                reaction_num_to_max_reactant_num.get(reaction_num, 0), len(
                reaction_log['building_block_ids']))
            for reactant_index, building_block_id in enumerate(reaction_log
                ['building_block_ids']):
    ...
    columns = ['smiles', 'node_id', 'num_expansions', 'rollout_num',
        'score', 'Q_value', 'num_reactions']
    for reaction_num in range(1, max_reaction_num + 1):
        columns.append(f'reaction_{reaction_num}_id')
        for reactant_num in range(1, reaction_num_to_max_reactant_num[
            reaction_num] + 1):
            columns.append(f'building_block_{reaction_num}_{reactant_num}_id')
            columns.append(
                f'building_block_{reaction_num}_{reactant_num}_smiles')
    save_path.parent.mkdir(parents=True, exist_ok=True)
    data = pd.DataFrame(data=[{'smiles': node.molecules[0], 'node_id': node
        .node_id, 'num_expansions': node.N, 'rollout_num': node.rollout_num,
        'score': node.P, 'Q_value': node.Q(), **construction_dict} for node,
        construction_dict in zip(nodes, construction_dicts)], columns=columns)
