...
def load_checkpoint(
    path: str, device: paddle.device = None, logger: logging.Logger = None
) -> MoleculeModel:
    """
    ...
    # Load model and args
    #state = paddle.load(path, map_location=lambda storage, loc: storage)
    state = paddle.load(path)
    
    args = TrainArgs()
    #arg_dic = Namespace(**args_dict)
...
def predict(model: MoleculeModel, data_loader: MoleculeDataLoader,
    ...
    return_unc_parameters: bool=False, dropout_prob: float=0.0) ->List[List
    [float]]:
    """
    ...
    for i, batch in enumerate(data_loader):
        ...
        (mol_batch, features_batch, _, _, atom_descriptors_batch, 
            atom_features_batch, bond_features_batch, _, _, _) = batch
        ...
        if model.loss_function == 'mve':
            ...
            if model.classification:
                batch_alphas = np.reshape(batch_preds, [tuple(batch_preds.
                    shape)[0], tuple(batch_preds.shape)[1] // 2, 2])
                batch_preds = batch_alphas[:, :, 1] / np.sum(batch_alphas,
                    axis=2)
            elif model.multiclass:
                batch_alphas = batch_preds
                batch_preds = batch_preds / np.sum(batch_alphas, axis=2,
                    keepdims=True)
