...
from synthemol.constants import BUILDING_BLOCKS_PATH, FINGERPRINT_TYPES, MODEL_TYPES, OPTIMIZATION_TYPES, REACTION_TO_BUILDING_BLOCKS_PATH, REAL_BUILDING_BLOCK_ID_COL, SCORE_COL, SMILES_COL
from synthemol.reactions import Reaction, REACTIONS, load_and_set_allowed_reaction_building_blocks, set_all_building_blocks
...
def generate(model_path: Path, model_type: MODEL_TYPES, save_dir: Path,
    building_blocks_path: Path=BUILDING_BLOCKS_PATH, fingerprint_type: (
    FINGERPRINT_TYPES)=None, reaction_to_building_blocks_path: (Path)=
    REACTION_TO_BUILDING_BLOCKS_PATH, building_blocks_id_column: str=
    REAL_BUILDING_BLOCK_ID_COL, building_blocks_score_column: str=SCORE_COL,
    building_blocks_smiles_column: str=SMILES_COL, reactions: tuple[
    Reaction]=REACTIONS, max_reactions: int=1, n_rollout: int=10,
    explore_weight: float=10.0, num_expand_nodes: (int)=None,
    optimization: OPTIMIZATION_TYPES='maximize', rng_seed: int=0,
    no_building_block_diversity: bool=False, store_nodes: bool=False,
    verbose: bool=False, replicate: bool=False) ->None:
    """Generate molecules combinatorially using a Monte Carlo tree search guided by a molecular property predictor.
