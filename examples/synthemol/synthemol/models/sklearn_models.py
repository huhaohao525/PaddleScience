...
def sklearn_predict(model: SKLEARN_MODEL_TYPES, fingerprints: np.ndarray
    ) ->np.ndarray:
    ...
    """
    if isinstance(model, RandomForestClassifier) or isinstance(model,
        MLPClassifier):
        preds = model.predict_proba(fingerprints)[:, 1]
    elif isinstance(model, RandomForestRegressor) or isinstance(model,
        MLPRegressor):
...
def sklearn_predict_on_molecule(model: SKLEARN_MODEL_TYPES, fingerprint: (
...
def sklearn_predict_on_molecule_ensemble(models: list[SKLEARN_MODEL_TYPES],
    fingerprint: (np.ndarray)=None) ->float:
    ...
    """
    return float(np.mean([sklearn_predict_on_molecule(model=model,
        fingerprint=fingerprint) for model in models]))
