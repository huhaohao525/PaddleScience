...
class Generator:
    ...
    def __init__(self, building_block_smiles_to_id: dict[str, int],
        max_reactions: int, scoring_fn: Callable[[str], float],
        explore_weight: float, num_expand_nodes: (int), optimization:
        OPTIMIZATION_TYPES, reactions: tuple[Reaction], rng_seed: int,
        no_building_block_diversity: bool, store_nodes: bool, verbose: bool,
        ...
        if replicate:
            self.all_building_blocks = list(dict.fromkeys(building_block for
                reaction in self.reactions for reactant in reaction.
                reactants for building_block in reactant.
                allowed_building_blocks))
        else:
            self.all_building_blocks = sorted(building_block for
                building_block in self.building_block_smiles_to_id if any(
                reactant.has_match(building_block) for reaction in
                reactions for reactant in reaction.reactants))
        ...
        self.rollout_num = 0
        self.root = Node(explore_weight=explore_weight, scoring_fn=
            scoring_fn, node_id=0, rollout_num=self.rollout_num)
    ...
    def get_next_building_blocks(self, molecules: tuple[str]) ->list[str]:
        ...
        for reaction in self.reactions:
            ...
            if len(molecules) >= reaction.num_reactants:
                continue
            reactant_matches_per_molecule = [reaction.get_reactant_matches(
                smiles=molecule) for molecule in molecules]
            for matched_reactant_indices in itertools.product(*
                ...
                if len(matched_reactant_indices) == len(molecules):
                    for index in sorted(reactant_indices -
                        matched_reactant_indices):
                        available_building_blocks += reaction.reactants[index
                            ].allowed_building_blocks
        available_building_blocks = list(dict.fromkeys(
            available_building_blocks))
        return available_building_blocks

    def get_reactions_for_molecules(self, molecules: tuple[str]) ->list[tuple
        ...
        for reaction in self.reactions:
            ...
            for matched_reactant_indices in itertools.product(*
                reactant_matches_per_molecule):
                if len(set(matched_reactant_indices)
                    ) == reaction.num_reactants:
                    molecule_to_reactant_index = dict(zip(molecules,
                        matched_reactant_indices))
                    matching_reactions.append((reaction,
                        molecule_to_reactant_index))
    ...
    def run_all_reactions(self, node: Node) ->list[Node]:
        ...
        for reaction, molecule_to_reactant_index in matching_reactions:
            molecules = sorted(node.molecules, key=lambda frag:
                molecule_to_reactant_index[frag])
            ...
            products = [Chem.MolToSmiles(Chem.RemoveHs(product[0])) for
                product in products]
            products = list(dict.fromkeys(product for product in products if
                product not in product_set))
            product_set |= set(products)
            reaction_log = {'reaction_id': reaction.id,
                'building_block_ids': tuple(self.
                building_block_smiles_to_id.get(molecule, -1) for molecule in
                molecules)}
            product_nodes += [Node(explore_weight=self.explore_weight,
                scoring_fn=self.scoring_fn, molecules=(product,),
                unique_building_block_ids=node.unique_building_block_ids,
                construction_log=node.construction_log + (reaction_log,),
                rollout_num=self.rollout_num) for product in products]
    ...
    def get_child_nodes(self, node: Node) ->list[Node]:
        ...
        if node.num_molecules == 0:
            ...
            next_building_blocks = self.get_next_building_blocks(molecules=
                node.molecules)
        if self.num_expand_nodes is not None and len(next_building_blocks
            ) > self.num_expand_nodes:
            next_building_blocks = random_choice(rng=self.rng, array=
                next_building_blocks, size=self.num_expand_nodes, replace=False
                )
        new_nodes += [Node(explore_weight=self.explore_weight, scoring_fn=
            self.scoring_fn, molecules=node.molecules + (
            next_building_block,), unique_building_block_ids=node.
            unique_building_block_ids | {next_building_block},
            construction_log=node.construction_log, rollout_num=self.
            rollout_num) for next_building_block in next_building_blocks]
    ...
    def compute_mcts_score(self, node: Node, total_visit_count: int) ->float:
        ...
        if self.building_block_diversity and node.num_molecules > 0:
            max_building_block_count = max(self.building_block_counts[
                building_block_id] for building_block_id in node.
    ...
    def rollout(self, node: Node) ->float:
        ...
        if self.verbose:
            ...
            print(
                f'Num unique building blocks = {len(node.unique_building_block_ids)}'
                )
        ...
        if node in self.node_to_children:
            ...
            child_nodes = [self.node_map.get(new_node, new_node) for
                new_node in child_nodes]
            for child_node in child_nodes:
                if (child_node.num_molecules == 1 and child_node not in
                    self.node_map):
                    child_node.node_id = len(self.node_map)
                    self.node_map[child_node] = child_node
                    self.building_block_counts.update(child_node.
                        unique_building_block_ids)
        ...
        selected_node = self.optimization_fn(child_nodes, key=partial(self.
            compute_mcts_score, total_visit_count=total_visit_count))
    ...
    def generate(self, n_rollout: int) ->list[Node]:
        ...
        nodes = [node for _, node in self.node_map.items() if node.
            num_molecules == 1 and node.num_reactions > 0 and rollout_start <=
            node.rollout_num < rollout_end]
        nodes = sorted(nodes, key=lambda node: (node.P, (1 if self.
            ascending_scores else -1) * node.node_id), reverse=not self.
            ascending_scores)
        return nodes
