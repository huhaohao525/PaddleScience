...
class Node:
    ...
    def __init__(self, explore_weight: float, scoring_fn: Callable[[str],
        float], node_id: (int)=None, molecules: (tuple[str])=
        None, unique_building_block_ids: (set[int])=None,
        construction_log: (tuple[dict[str, Any]])=None, rollout_num:
        (int)=None) ->None:
        ...
        self.unique_building_block_ids = (unique_building_block_ids if 
            unique_building_block_ids is not None else set())
        self.construction_log = (construction_log if construction_log is not
            None else tuple())
        self.W = 0.0
        self.N = 0
    ...
    @classmethod
    def compute_score(cls, molecules: tuple[str], scoring_fn: Callable[[str
        ], float]) ->float:
        ...
        """
        return sum(scoring_fn(molecule) for molecule in molecules) / len(
            molecules) if len(molecules) > 0 else 0.0

    @cached_property
    def P(self) ->float:
        """The property score of this Node. (Note: The value is cached, so it assumes the Node is immutable.)"""
        return self.compute_score(molecules=self.molecules, scoring_fn=self
            .scoring_fn)
