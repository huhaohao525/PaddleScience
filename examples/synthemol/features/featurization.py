...
import numpy as np
from myrdkit.myrdkit import make_mol
...
def atom_features_zeros(atom: Chem.rdchem.Atom) ->List[Union[bool, int, float]
    ...
    if atom is None:
        ...
        features = onek_encoding_unk(atom.GetAtomicNum() - 1, PARAMS.
            ATOM_FEATURES['atomic_num']) + [0] * (PARAMS.ATOM_FDIM - PARAMS
            .MAX_ATOMIC_NUM - 1)
    return features
...
class MolGraph:
    ...
    * :code:`reaction_mode`:  Reaction mode to construct atom and bond feature vectors
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        atom_features_extra: np.ndarray=None, bond_features_extra: np.
        ndarray=None, overwrite_default_atom_features: bool=False,
...
class BatchMolGraph:
    ...
    * :code:`a2a`: (Optional): A mapping from an atom index to neighboring atom indices.
    ...
    def get_components(self, atom_messages: bool=False) ->Tuple[paddle.
        Tensor, paddle.Tensor, paddle.Tensor, paddle.Tensor, paddle.Tensor,
...
def mol2graph(mols: Union[List[str], List[Chem.Mol], List[Tuple[Chem.Mol,
    Chem.Mol]]], atom_features_batch: List[np.array]=(None,),
    bond_features_batch: List[np.array]=(None,),
    overwrite_default_atom_features: bool=False,
    overwrite_default_bond_features: bool=False) ->BatchMolGraph:
    """
    ...
    return BatchMolGraph([MolGraph(mol, af, bf,
        ...
        overwrite_default_bond_features=overwrite_default_bond_features) for
        mol, af, bf in zip_longest(mols, atom_features_batch,
        bond_features_batch)])
