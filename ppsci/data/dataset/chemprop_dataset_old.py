...
def is_mol(mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]]):
...
def atom_features(atom: Chem.rdchem.Atom, functional_groups: List[int]=None
    ...
    if atom is None:
        ...
        features = onek_encoding_unk(atom.GetAtomicNum() - 1, PARAMS.
            ATOM_FEATURES['atomic_num']) + onek_encoding_unk(atom.
            GetTotalDegree(), PARAMS.ATOM_FEATURES['degree']
            ) + onek_encoding_unk(atom.GetFormalCharge(), PARAMS.
            ATOM_FEATURES['formal_charge']) + onek_encoding_unk(int(atom.
            GetChiralTag()), PARAMS.ATOM_FEATURES['chiral_tag']
            ) + onek_encoding_unk(int(atom.GetTotalNumHs()), PARAMS.
            ATOM_FEATURES['num_Hs']) + onek_encoding_unk(int(atom.
            GetHybridization()), PARAMS.ATOM_FEATURES['hybridization']) + [
...
def get_atom_fdim(overwrite_default_atom: bool=False, is_reaction: bool=False
    ...
    if PARAMS.REACTION_MODE:
        ...
        return (not overwrite_default_atom
            ) * PARAMS.ATOM_FDIM + PARAMS.EXTRA_ATOM_FDIM

def get_bond_fdim(atom_messages: bool=False, overwrite_default_bond: bool=
    ...
    if PARAMS.REACTION_MODE:
        return (not overwrite_default_bond
            ) * PARAMS.BOND_FDIM + is_reaction * PARAMS.EXTRA_BOND_FDIM + (not
            atom_messages) * get_atom_fdim(overwrite_default_atom=
            overwrite_default_atom, is_reaction=is_reaction)
    else:
        return (not overwrite_default_bond
            ) * PARAMS.BOND_FDIM + PARAMS.EXTRA_BOND_FDIM + (not atom_messages
...
class MolGraph:
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        if type(mol) == str:
            if self.is_reaction:
                mol = make_mol(mol.split('>')[0], self.is_explicit_h, self.
        ...
        if not self.is_reaction:
            ...
            if self.reaction_mode in ['reac_diff', 'prod_diff', 'reac_prod']:
                f_atoms_reac = [atom_features(atom) for atom in mol_reac.
                    GetAtoms()] + [atom_features_zeros(mol_prod.
                    GetAtomWithIdx(index)) for index in pio]
                f_atoms_prod = [(atom_features(mol_prod.GetAtomWithIdx(
                    ri2pi[atom.GetIdx()])) if atom.GetIdx() not in rio else
                    atom_features_zeros(atom)) for atom in mol_reac.GetAtoms()
                    ] + [atom_features(mol_prod.GetAtomWithIdx(index)) for
                    index in pio]
            else:
                f_atoms_reac = [atom_features(atom) for atom in mol_reac.
                    GetAtoms()] + [atom_features(mol_prod.GetAtomWithIdx(
                    index)) for index in pio]
                f_atoms_prod = [(atom_features(mol_prod.GetAtomWithIdx(
                    ri2pi[atom.GetIdx()])) if atom.GetIdx() not in rio else
                    atom_features(atom)) for atom in mol_reac.GetAtoms()] + [
                    atom_features(mol_prod.GetAtomWithIdx(index)) for index in
                    pio]
            ...
            if self.reaction_mode in ['reac_prod', 'reac_prod_balance']:
                self.f_atoms = [(x + y[PARAMS.MAX_ATOMIC_NUM + 1:]) for x,
...
class BatchMolGraph:
    ...
    def __init__(self, mol_graphs: List[MolGraph]):
        ...
        self.atom_fdim = get_atom_fdim(overwrite_default_atom=self.
            overwrite_default_atom_features, is_reaction=self.is_reaction)
        self.bond_fdim = get_bond_fdim(overwrite_default_bond=self.
    ...
    for s, reaction, keep_h, add_h in zip(smiles, reaction_list,
        ...
        if reaction:
            mol.append(SMILES_TO_MOL[s] if s in SMILES_TO_MOL else (
                make_mol(s.split('>')[0], keep_h, add_h), make_mol(s.split(
                '>')[-1], keep_h, add_h)))
        else:
