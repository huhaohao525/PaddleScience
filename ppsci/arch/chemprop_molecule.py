...
from examples.synthemol.features import BatchMolGraph, get_atom_fdim, get_bond_fdim, mol2graph
from examples.synthemol.nn_utils import index_select_ND, get_activation_function
...
class MPNEncoder(paddle.nn.Layer):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int, bond_fdim: int,
        hidden_size: int=None, bias: bool=None, depth: int=None):
        """
       ...
       """
        ...
        # Cached zeros
        self.cached_zero_vector = (paddle.base.framework.EagerParamBase.
            from_tensor(tensor=paddle.zeros(shape=self.hidden_size),
            trainable=False))
        
        # Input
        input_dim = self.atom_fdim if self.atom_messages else self.bond_fdim
        self.W_i = paddle.nn.Linear(in_features=input_dim, out_features=
            self.hidden_size, bias_attr=self.bias)
        if self.atom_messages:
            w_h_input_size = self.hidden_size + self.bond_fdim
        else:
            w_h_input_size = self.hidden_size
        self.W_h = paddle.nn.Linear(in_features=w_h_input_size,
            out_features=self.hidden_size, bias_attr=self.bias)
        self.W_o = paddle.nn.Linear(in_features=self.atom_fdim + self.
            hidden_size, out_features=self.hidden_size)
        if args.atom_descriptors == 'descriptor':
            self.atom_descriptors_size = args.atom_descriptors_size
            self.atom_descriptors_layer = paddle.nn.Linear(in_features=self
                .hidden_size + self.atom_descriptors_size, out_features=
                self.hidden_size + self.atom_descriptors_size)

    def forward(self, mol_graph, atom_descriptors_batch:
        List[np.ndarray]=None) ->paddle.float32:
        """
        ...
        if atom_descriptors_batch is not None:
            atom_descriptors_batch = [np.zeros([1, tuple(
                atom_descriptors_batch[0].shape)[1]])] + atom_descriptors_batch
            atom_descriptors_batch = paddle.to_tensor(data=np.concatenate(
                atom_descriptors_batch, axis=0)).astype(dtype='float32').to(
                self.device)
        #print(self.device)
        ...
        for depth in range(self.depth - 1):
            ...
            if self.atom_messages:
                ...
                nei_message = paddle.concat(x=(nei_a_message, nei_f_bonds),
                    axis=2)
        ...
        if atom_descriptors_batch is not None:
            ...
            atom_hiddens = paddle.concat(x=[atom_hiddens,
                atom_descriptors_batch], axis=1)
        ...
        for i, (a_start, a_size) in enumerate(a_scope):
            if a_size == 0:
                ...
                start_0 = atom_hiddens.shape[0
                    ] + a_start if a_start < 0 else a_start
                cur_hiddens = paddle.slice(atom_hiddens, [0], [start_0], [
                    start_0 + a_size])
                mol_vec = cur_hiddens
...
class MPN(paddle.nn.Layer):
    ...
    def __init__(self, args: TrainArgs, atom_fdim: int=None, bond_fdim: int
        =None):
        """
        :param args: A :class:`~chemprop.args.TrainArgs` object containing model arguments.
        :param atom_fdim: Atom feature vector dimension.
        ...
        self.atom_fdim = atom_fdim or get_atom_fdim(overwrite_default_atom=
            args.overwrite_default_atom_features, is_reaction=self.reaction or
            self.reaction_solvent)
        self.bond_fdim = bond_fdim or get_bond_fdim(overwrite_default_atom=
            args.overwrite_default_atom_features, overwrite_default_bond=
            args.overwrite_default_bond_features, atom_messages=args.
            atom_messages, is_reaction=self.reaction or self.reaction_solvent)
        self.features_only = args.features_only
        self.use_input_features = args.use_input_features
        self.device = args.device
        self.atom_descriptors = args.atom_descriptors
        self.overwrite_default_atom_features = (args.
            overwrite_default_atom_features)
        self.overwrite_default_bond_features = (args.
            overwrite_default_bond_features)
        if self.features_only:
            return
        if not self.reaction_solvent:
            if args.mpn_shared:
                self.encoder = paddle.nn.LayerList(sublayers=[MPNEncoder(
                    args, self.atom_fdim, self.bond_fdim)] * args.
                    number_of_molecules)
            else:
                self.encoder = paddle.nn.LayerList(sublayers=[MPNEncoder(
                    args, self.atom_fdim, self.bond_fdim) for _ in range(
                    args.number_of_molecules)])
        else:
            self.encoder = MPNEncoder(args, self.atom_fdim, self.bond_fdim)
            self.atom_fdim_solvent = get_atom_fdim(overwrite_default_atom=
                args.overwrite_default_atom_features, is_reaction=False)
            self.bond_fdim_solvent = get_bond_fdim(overwrite_default_atom=
                args.overwrite_default_atom_features,
                overwrite_default_bond=args.overwrite_default_bond_features,
                atom_messages=args.atom_messages, is_reaction=False)
            self.encoder_solvent = MPNEncoder(args, self.atom_fdim_solvent,
                self.bond_fdim_solvent, args.hidden_size_solvent, args.
                bias_solvent, args.depth_solvent)

    def forward(self, batch: Union[List[List[str]], List[List[Chem.Mol]],
        List[List[Tuple[Chem.Mol, Chem.Mol]]], List[BatchMolGraph]],
        features_batch: List[np.ndarray]=None, atom_descriptors_batch: List
        [np.ndarray]=None, atom_features_batch: List[np.ndarray]=None,
        bond_features_batch: List[np.ndarray]=None) ->paddle.float32:
        """
        ...
        if self.use_input_features:
            features_batch = paddle.to_tensor(data=np.stack(features_batch)
        ...
        if self.atom_descriptors == 'descriptor':
            ...
            encodings = [enc(ba, atom_descriptors_batch) for enc, ba in zip
                (self.encoder, batch)]
...
def index_select_ND(source: paddle.Tensor, index: paddle.Tensor
    ) ->paddle.Tensor:
    """
...
class NoamLR(paddle.optimizer.lr.LRScheduler):
    ...
    def __init__(self, optimizer: paddle.optimizer.Optimizer, warmup_epochs:
        ...
        """
        if not len(optimizer._param_groups) == len(warmup_epochs) == len(
            total_epochs) == len(init_lr) == len(max_lr) == len(final_lr):
            raise ValueError(
        ...
        self.current_step = 0
        self.lr = init_lr
        self.warmup_steps = (self.warmup_epochs * self.steps_per_epoch).astype(
            int)
        self.total_steps = self.total_epochs * self.steps_per_epoch
        self.linear_increment = (self.max_lr - self.init_lr
            ) / self.warmup_steps
        self.exponential_gamma = (self.final_lr / self.max_lr) ** (1 / (
...
class MoleculeModel(paddle.nn.Layer):
    ...
    def create_encoder(self, args: TrainArgs) ->None:
        ...
        if args.checkpoint_frzn is not None:
            if args.freeze_first_only:
                for param in list(self.encoder.encoder.children())[0
                    ].parameters():
    ...
    def create_ffn(self, args: TrainArgs) ->None:
        ...
        if args.ffn_num_layers == 1:
            ffn = [dropout, paddle.nn.Linear(in_features=first_linear_dim,
                out_features=self.output_size)]
        else:
            ffn = [dropout, paddle.nn.Linear(in_features=first_linear_dim,
                out_features=args.ffn_hidden_size)]
            for _ in range(args.ffn_num_layers - 2):
                ffn.extend([activation, dropout, paddle.nn.Linear(
                    in_features=args.ffn_hidden_size, out_features=args.
                    ffn_hidden_size)])
            ffn.extend([activation, dropout, paddle.nn.Linear(in_features=
                args.ffn_hidden_size, out_features=self.output_size)])
        ...
        if args.checkpoint_frzn is not None:
            if args.frzn_ffn_layers > 0:
                for param in list(self.ffn.parameters())[0:2 * args.
                    frzn_ffn_layers]:
                    param.stop_gradient = not False

    def fingerprint(self, batch: Union[List[List[str]], List[List[Chem.Mol]
        ], List[List[Tuple[Chem.Mol, Chem.Mol]]], List[BatchMolGraph]],
        features_batch: List[np.ndarray]=None, atom_descriptors_batch: List
        [np.ndarray]=None, atom_features_batch: List[np.ndarray]=None,
        bond_features_batch: List[np.ndarray]=None, fingerprint_type: str='MPN'
        ) ->paddle.Tensor:
        """
    ...
    def forward(self, batch: Union[List[List[str]], List[List[Chem.Mol]],
        ...
        features_batch = batch['features_batch'] #paddle.to_tensor(features_batch, dtype="float32"),
        atom_descriptors_batch = batch['atom_descriptors_batch'] #paddle.to_tensor(atom_descriptors_batch, dtype="float32"),
        atom_features_batch = batch['atom_features_batch'] #paddle.to_tensor(atom_features_batch, dtype="float32"),
        bond_features_batch = batch['bond_features_batch'] #paddle.to_tensor(bond_features_batch, dtype="float32"),
        batch = mol_batch

        output = self.ffn(self.encoder(batch, features_batch,
            atom_descriptors_batch, atom_features_batch, bond_features_batch))
        if self.classification and not (self.training and self.
            no_training_normalization) and self.loss_function != 'dirichlet':
            output = self.sigmoid(output)
        if self.multiclass:
            output = output.reshape((tuple(output.shape)[0], -1, self.
                num_classes))
            if not (self.training and self.no_training_normalization
                ) and self.loss_function != 'dirichlet':
                output = self.multiclass_softmax(output)
        if self.loss_function == 'mve':
            means, variances = split(x=output, num_or_sections=
                tuple(output.shape)[1] // 2, axis=1)
            variances = self.softplus(variances)
            output = paddle.concat([means, variances], axis=1)

        if self.loss_function == 'evidential':
            means, lambdas, alphas, betas = split(x=output,
                num_or_sections=tuple(output.shape)[1] // 4, axis=1)
